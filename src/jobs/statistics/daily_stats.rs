...
async fn get_queue_statistics(db: &sea_orm::DatabaseConnection) -> Result<QueueStatistics> {
    ...
    let average_wait_time_minutes = if !waiting_entries.is_empty() {
        ...
        let total_wait_minutes: i64 = waiting_entries.iter()
...
async fn get_daily_match_statistics(db: &sea_orm::DatabaseConnection, today: chrono::NaiveDate) -> Result<MatchStatistics> {
    ...
    let total_matches_today = entity::r#match::Entity::find()
        .filter(
            entity::r#match::Column::StartedAt.gte(start_of_day)
    ...
    let completed_matches_today = entity::r#match::Entity::find()
        .filter(
            entity::r#match::Column::CompletedAt.gte(start_of_day)
    ...
    let average_match_duration_minutes = if !completed_matches.is_empty() {
        let total_duration_minutes: i64 = completed_matches.iter()
...
async fn get_report_statistics(db: &sea_orm::DatabaseConnection) -> Result<ReportStatistics> {
    ...
    let reports_today = entity::report::Entity::find()
        .filter(
            entity::report::Column::CreatedAt.gte(start_of_day)
    ...
    let low_severity = entity::report::Entity::find()
        .filter(
            entity::report::Column::Status.eq("Active")
    ...
    Ok(ReportStatistics {
        pending_reports,
        reports_today,
        reports_by_severity: (low_severity, medium_severity, high_severity, critical_severity),
    })
}