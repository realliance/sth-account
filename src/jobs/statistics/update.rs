...
pub async fn run(db: &sea_orm::DatabaseConnection) -> Result<()> {
    ...
    info!("Statistics update complete - updated {} users and {} bots", updated_users, updated_bots);
    Ok(())
}
...
async fn update_user_statistics(db: &sea_orm::DatabaseConnection) -> Result<u64> {
    ...
    for user in users {
        ...
        if let Some(existing) = existing_stats {
            ...
            active_model.peak_mmr = Set(stats.peak_mmr.or(existing_peak).or(Some(user.matchmaking_rank)));
...
async fn update_bot_statistics(db: &sea_orm::DatabaseConnection) -> Result<u64> {
    ...
    for bot in bots {
        ...
        if let Some(existing) = existing_stats {
            ...
            active_model.fourth_place = Set(stats.fourth_place);
            active_model.average_score = Set(stats.average_score);
            active_model.peak_mmr = Set(stats.peak_mmr.or(existing_peak).or(Some(bot.matchmaking_rank)));
            active_model.current_streak = Set(stats.current_streak);
            active_model.last_game_at = Set(stats.last_game_at);
...
async fn calculate_user_stats_from_matches(db: &sea_orm::DatabaseConnection, user_id: uuid::Uuid) -> Result<UserStats> {
    // Get all matches where this user participated
    let matches = entity::r#match::Entity::find()
        .filter(
            entity::r#match::Column::Participant1Id.eq(user_id)
    ...
    for match_data in &sorted_matches {
        ...
        let (score, placement) = if match_data.participant1_id == user_id {
            (match_data.participant1_score.unwrap_or(0), get_placement_from_scores(&match_data, match_data.participant1_score.unwrap_or(0)))
        } else if match_data.participant2_id == user_id {
            (match_data.participant2_score.unwrap_or(0), get_placement_from_scores(&match_data, match_data.participant2_score.unwrap_or(0)))
        } else if match_data.participant3_id == user_id {
            (match_data.participant3_score.unwrap_or(0), get_placement_from_scores(&match_data, match_data.participant3_score.unwrap_or(0)))
        } else if match_data.participant4_id == Some(user_id) {
            (match_data.participant4_score.unwrap_or(0), get_placement_from_scores(&match_data, match_data.participant4_score.unwrap_or(0)))
        } else {
            continue;
        };
    ...
    if !scores.is_empty() {
        ...
        stats.average_score = Some(Decimal::new(sum as i64, 0) / Decimal::new(scores.len() as i64, 0));
    }
    
    Ok(stats)
}

async fn calculate_bot_stats_from_matches(db: &sea_orm::DatabaseConnection, bot_id: uuid::Uuid) -> Result<BotStats> {
    // Similar to user stats but for bots
    let matches = entity::r#match::Entity::find()
        .filter(
            entity::r#match::Column::Participant1Id.eq(bot_id)
