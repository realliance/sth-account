...
async fn recalculate_user_mmr(db: &sea_orm::DatabaseConnection) -> Result<u64> {
    ...
    for match_data in sorted_matches {
        ...
        let user_participants: Vec<_> = participants.iter()
...
async fn recalculate_bot_mmr(db: &sea_orm::DatabaseConnection) -> Result<u64> {
    ...
    for match_data in sorted_matches {
        let participants = get_match_participants(&match_data);
        let bot_participants: Vec<_> = participants.iter()
...
fn get_match_participants(match_data: &entity::r#match::Model) -> Vec<Participant> {
    ...
    scores_and_ids.push((match_data.participant1_id, match_data.participant1_score.unwrap_or(0)));
    scores_and_ids.push((match_data.participant2_id, match_data.participant2_score.unwrap_or(0)));
    scores_and_ids.push((match_data.participant3_id, match_data.participant3_score.unwrap_or(0)));
...
fn calculate_mmr_changes(participants: &[&Participant], current_mmrs: &HashMap<uuid::Uuid, i32>) -> HashMap<uuid::Uuid, i32> {
    ...
    for participant in participants {
        let current_mmr = current_mmrs.get(&participant.id).copied().unwrap_or(INITIAL_MMR) as f64;
        
        // Calculate expected score based on placement
        // First place gets 1.0, last place gets 0.0, others interpolated
        let actual_score = (participant_count - participant.placement as f64 + 1.0) / participant_count;
        
        // Calculate average opponent MMR
        let opponent_mmr_sum: i32 = participants.iter()
