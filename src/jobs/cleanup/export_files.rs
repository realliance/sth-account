...
pub async fn run(db: &sea_orm::DatabaseConnection) -> Result<()> {
    ...
    let expired_exports = entity::data_export_requests::Entity::find()
        .filter(
            entity::data_export_requests::Column::Status
                .eq("Completed")
    ...
    info!(
        "Cleaned up {} expired export files older than 7 days",
        deleted_count
    );
    Ok(())
}
...
mod tests {
    ...
    async fn test_cleanup_export_files_removes_expired_exports() {
        ...
        expired_export
            .insert(&db)
            .await
            .expect("Failed to insert test export");
        ...
        assert_eq!(
            remaining_exports.len(),
            0,
            "Expired exports should be deleted"
        );

        // Verify the file was deleted from filesystem
        assert!(
            !temp_file_path.exists(),
            "Export file should be deleted from filesystem"
        );
    }
